...
@interface PDHeaderRefreshView ()<CAAnimationDelegate>
        ...
        if (!self.associatedScrollView.tracking)
        {
            ...
            if (self.isFlash) {
                ...
                [UIView animateWithDuration:0.3 animations:^{
                    ...
                    weakSelf.associatedScrollView.contentInset = UIEdgeInsetsMake(strongSelf->PDPullToRefreshViewHeight + strongSelf->originOffset, 0, 0, 0);
...
-(void)startRefreshing
{
    ...
    [UIView animateWithDuration:0.3 animations:^{
        ...
        weakSelf.associatedScrollView.contentInset = UIEdgeInsetsMake(PDPullToRefreshViewHeight + originOffset, 0, 0, 0);
        
    } completion:^(BOOL finished) {
        __strong __typeof__(weakSelf) strongSelf = weakSelf;
        weakSelf.handleBlock();
       strongSelf -> isStop = YES;
//        [self stopRefreshing];
...
//    self.progress = 1.0;
    ...
    [UIView animateWithDuration:0.1 delay:0 options:UIViewAnimationOptionCurveEaseInOut animations:^{
        weakSelf.alpha = 0.0f;
        weakSelf.associatedScrollView.contentInset = UIEdgeInsetsMake(originOffset+0.1, 0, 0, 0);
    } completion:^(BOOL finished) {
        weakSelf.alpha = 1.0f;
        [weakSelf stopAnimating];
        [weakSelf addPullAnimation];
        weakSelf.pathLayer.strokeEnd = 0.0;
    }];
}
...
- (void)addRefreshAnimation
{
    ...
    gradientLayer.colors           = @[(__bridge id)pColor,
...
//        group.delegate = self;
        [group setValue:gradientLayer forKey:@"parentLayer"];

        [gradientLayer addAnimation:group forKey:kAnimationKey];
    }
}
-(void)animationDidStop:(CAAnimation *)animation finished:(BOOL)flag {
    NSLog(@"There were %d sublayers",[self.animationLayer.sublayers count]);
    CAShapeLayer *layer = [animation valueForKey:@"parentLayer"];
    [layer removeAllAnimations];
    [layer removeFromSuperlayer];
    NSLog(@"There are now %d sublayers",[self.animationLayer.sublayers count]);
}
/** 结束动画 */
- (void)stopAnimating
{
